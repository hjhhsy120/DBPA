...
class dataset:
        ...
        for fname in file_list:
            ...
            if 'setknob' in fname:
                for id, d in enumerate(data):
                    ...
                    i = 0
                    while i < d_len:
                        self.append_data(d[0][i+1:i+1+time_cnt], d[1], id in test_ids)
                        i+=12
            else:
                for id, d in enumerate(data):
                    self.append_data(d[0][1:1+time_cnt], d[1], id in test_ids)
    ...
    def load_files2(self, file_list, time_cnt=10):
        for fname in file_list:
            ...
            test_ids = self.gen_test_ids(len(data))
            for id, d in enumerate(data):
                self.append_data(d[0][d[1][0]+1:d[1][0]+1+time_cnt], d[2], id in test_ids)
    ...
    def load_files4(self, file_list, time_cnt=10):
        for fname in file_list:
            ...
            test_ids = self.gen_test_ids(len(data))
            for id, d in enumerate(data):
                self.append_data(d[0][1:1+time_cnt], 'normal', id in test_ids)
